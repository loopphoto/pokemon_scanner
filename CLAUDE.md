...
- **`app.js`** — all app logic (ES module). Key pieces:
  ...
  - **Collection item shape**: `{ id, name, image, set, rarity, types, price, authenticity }` where `authenticity` is `"real"` or `"fake"` (defaults to `"real"` if absent, for items saved before that field existed).
